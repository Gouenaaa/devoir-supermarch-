...
public class FrmPrincipal extends javax.swing.JFrame {
    ...
    private void tblSecteursMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblSecteursMouseClicked
        ...
        numSecteur = Integer.parseInt(tblSecteurs.getValueAt(tblSecteurs.getSelectedRow(), 0).toString());
        for(Rayon r : fm.GetAllRayonsByIdsecteur(numSecteur)){
            v = new Vector();
            v.add(r.getIdRayon());
            v.add(r.getNomRayon());
            dtmRayons.addRow(v);
    ...
    private void AfficherEmployes(){
        while(dtmEmployes.getRowCount()!=0){
            ...
            numRayon = Integer.parseInt(tblRayons.getValueAt(tblRayons.getSelectedRow(), 0).toString());
    ...
    private void btnModifierMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnModifierMouseClicked
        if(tblSecteurs.getSelectedRow() < 0){
            JOptionPane.showMessageDialog(this, "Veuillez sélectionner un secteur");
        }
        else{
            if(tblRayons.getSelectedRow() < 0){
                JOptionPane.showMessageDialog(this, "Veuillez sélectionner un rayon");
            }
            else{
                if(tblEmployes.getSelectedRow() < 0){
                    JOptionPane.showMessageDialog(this, "Veuillez sélectionner un employé");
                }
                else{
                    if(txtNouveauTemps.getText().compareTo("") == 0){
                        JOptionPane.showMessageDialog(this, "Veuillez saisir un nouveau temps");
                    }
                    else{
                        int codeEmploye = Integer.parseInt(tblEmployes.getValueAt(tblEmployes.getSelectedRow(), 0).toString());
                        String date = tblEmployes.getValueAt(tblEmployes.getSelectedRow(), 2).toString();
                        int temps = Integer.parseInt(txtNouveauTemps.getText());
                        fm.ModifierTemps(codeEmploye, numRayon, date, temps);
                        AfficherEmployes();
                    }
                }
            }
    ...
    private void btnInsererTempsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnInsererTempsMouseClicked
        if(tblSecteurs.getSelectedRow() < 0){
            ...
            if(tblRayons.getSelectedRow() < 0){
                ...
                if(txtNouveauTemps.getText().compareTo("") == 0){
                    JOptionPane.showMessageDialog(this, "Veuillez saisir un temps");
                }
                else{
                    String nomE = cboEmployes.getSelectedItem().toString();
                    int numE = fm.GetIdEmployeByNom(nomE);
                    int temps = Integer.parseInt(txtNouveauTemps.getText());
                    if(fm.TestTravailler(numE, numRayon)){
                        fm.InsererTemps(numE, numRayon, temps);
                        AfficherEmployes();
                    }
                    else{
                        JOptionPane.showMessageDialog(this, "Vous ne pouvez pas insérer un nouveau temps pour "+nomE+" aujourd'hui");
                    }
                    cboEmployes.setSelectedIndex(0);
                }
            }
        }
